...
type WorkoutSessionState = ReturnType<typeof useWorkoutSession>;
type WorkoutTimerState = ReturnType<typeof useRestTimer>;
type ProgramExercise = ProgramWithExercises["exercises"][number];
...
type WorkoutDataOptions = {
  params: Promise<{ id: string }>;
  onProgramLoaded: (program: ProgramWithExercises) => void;
  setProgram: (program: ProgramWithExercises | null) => void;
  setLoading: (loading: boolean) => void;
  setSessionId: (sessionId: string | null) => void;
};

function useWorkoutDetailData({
  params,
  onProgramLoaded,
  setProgram,
  setLoading,
  ...
  useEffect(() => {
    ...
    fetchProgram();
  }, [params, onProgramLoaded, setProgram, setLoading, setSessionId]);
}
...
export default function WorkoutDetailPage({ params }: Props) {
  ...
  useWorkoutDetailData({
    params,
    onProgramLoaded: session.actions.initializeInputs,
    setProgram,
    setLoading,
    setSessionId,
  });
  ...
  if (!program) {
    return <WorkoutNotFound loading={loading} />;
  }

  const hasIncompleteSets = session.actions.hasIncompleteSets();
    ...
    <>
      <div className="min-h-screen px-4 pt-6 pb-32 bg-gray-50">
        <div className="max-w-md mx-auto">
          ...
          <WorkoutProgress currentIndex={session.currentIndex} total={program.exercises.length} />
          <ExerciseList program={program} session={session} exerciseRefs={exerciseRefs} />
        </div>
      </div>
      ...
      {timer.isOpen && <RestTimerModal timer={timer} />}

      {showSkipConfirm && (
        <SkipConfirmModal
          onCancel={() => setShowSkipConfirm(false)}
          onConfirm={confirmSkipAndMoveNext}
        />
      )}
...
function WorkoutNotFound({ loading }: WorkoutNotFoundProps) {
  return (
      ...
      <div className="max-w-md mx-auto">
        ...
        {!loading && (
          <Card padding="lg" className="text-center">
            ...
            <Link href="/workout">
              <Button variant="primary">프로그램 목록으로 돌아가기</Button>
            </Link>
          </Card>
        )}
      </div>
...
function WorkoutHeader({ program }: WorkoutHeaderProps) {
  return (
    <header className="flex items-center mb-4">
      ...
      <div className="flex-1">
        ...
        {program.rpe ? (
          <p className="text-sm text-blue-600 font-medium mt-1">RPE: {program.rpe}</p>
        ) : (
          <p className="text-sm text-gray-400 mt-1">RPE: 없음</p>
        )}
      </div>
    </header>
  );
...
function WorkoutProgress({ currentIndex, total }: WorkoutProgressProps) {
  const progressPercentage = total === 0 ? 0 : (currentIndex / total) * 100;
...
function ExerciseList({ program, session, exerciseRefs }: ExerciseListProps) {
  return (
...
type ExerciseCardProps = {
  ...
  isLocked: boolean;
  session: WorkoutSessionState;
  exerciseRef: (element: HTMLDivElement | null) => void;
};
...
function ExerciseCard({
  ...
  return (
    ...
    >
            ...
            <div>
              <h3
                className={`font-semibold ${
                  isCompleted ? "text-green-800" : isCurrent ? "text-blue-800" : "text-slate-500"
                }`}
              >
                {exercise.name}
              </h3>
              <div className="flex items-center gap-2 mt-0.5">
                ...
                {exercise.rest_seconds && (
                  <>
                    ...
                    <span className="text-xs text-slate-500">휴식 {exercise.rest_seconds}초</span>
                  </>
                )}
              </div>
            </div>
      ...
      {isCurrent && <ExerciseInputPanel exercise={exercise} session={session} />}

      {isCompleted && <ExerciseSummary exercise={exercise} session={session} />}
    </div>
  );
}

type ExerciseInputPanelProps = {
  exercise: ProgramExercise;
  session: WorkoutSessionState;
};

function ExerciseInputPanel({ exercise, session }: ExerciseInputPanelProps) {
  return (
    <div className="p-4 border-t border-gray-100">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <input
                      ...
                      onChange={(e) =>
                        session.actions.updateInput(exercise.id, setIndex, "weight", e.target.value)
                      }
                  ...
                  <div>
                    <input
                      ...
                      value={setData?.reps || ""}
                      onChange={(e) =>
                        session.actions.updateInput(exercise.id, setIndex, "reps", e.target.value)
                      }
                      disabled={isSetCompleted}
                      className={`w-full p-2 border rounded-lg text-center font-medium focus:outline-none focus:ring-2 ${
                        isSetCompleted
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <input
                      ...
                      value={setData?.time || ""}
                      onChange={(e) =>
                        session.actions.updateInput(exercise.id, setIndex, "time", e.target.value)
                      }
                      disabled={isSetCompleted}
                      className={`w-full p-2 border rounded-lg text-center font-medium focus:outline-none focus:ring-2 ${
                        isSetCompleted
      ...
      <div className="mt-4">
        <label className="block text-sm font-medium text-slate-700 mb-2">운동 메모</label>
        <textarea
...
function ExerciseSummary({ exercise, session }: ExerciseSummaryProps) {
  const completedSets = session.inputs[exercise.id]?.filter((set) => set.completed) || [];
...
function WorkoutActions({ isLastExercise, hasIncompleteSets, isSaving, onNext }: WorkoutActionsProps) {
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 shadow-lg">
      <div className="max-w-md mx-auto">
        {hasIncompleteSets && !isLastExercise ? (
          <>
            <Button
              onClick={onNext}
              isLoading={isSaving}
          ...
          >
            <Check className="w-5 h-5" />
            {isLastExercise ? "운동 기록 저장" : "다음 운동으로"}
          </Button>
        )}
      </div>
...
function RestTimerModal({ timer }: RestTimerModalProps) {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl p-8 max-w-sm w-full text-center">
        ...
        <Button onClick={timer.skip} fullWidth size="lg" variant="primary" className="shadow-md">
          휴식 건너뛰기
        </Button>
      </div>
    </div>
...
function SkipConfirmModal({ onCancel, onConfirm }: SkipConfirmModalProps) {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl p-8 max-w-sm w-full text-center">
        <div className="mb-6">
...
function CompletionModal({ program, session, onDashboard, onHome }: CompletionModalProps) {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl p-6 max-w-md w-full max-h-[80vh] overflow-y-auto">
        ...
        <div className="flex gap-3">
          ...
          <Button onClick={onHome} variant="primary" className="flex-1">
            홈으로 이동
          </Button>
        </div>
      </div>
